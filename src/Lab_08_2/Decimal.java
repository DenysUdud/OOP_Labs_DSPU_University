...
class Decimal extends Integer {
    ...
    public Integer add(Integer other) {
        if (other instanceof Decimal) {
            ...
            int maxLength = Math.max(digits.length, decimal.digits.length);
            int[] resultDigits = new int[maxLength + 1];
            int carry = 0;

            for (int i = 0; i < maxLength; i++) {
                int digit1 = i < digits.length ? digits[i] : 0;
                int digit2 = i < decimal.digits.length ? decimal.digits[i] : 0;
            ...
            return new Decimal(resultDigits);
